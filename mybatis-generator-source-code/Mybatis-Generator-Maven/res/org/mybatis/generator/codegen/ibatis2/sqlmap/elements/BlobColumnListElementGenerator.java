...
// Source File Name:   BlobColumnListElementGenerator.java
...
public class BlobColumnListElementGenerator extends AbstractXmlElementGenerator
{
    ...
    public void addElements(XmlElement parentElement)
    {
        ...
        for(Iterator iter = introspectedTable.getBLOBColumns().iterator(); iter.hasNext();)
        {
            sb.append(Ibatis2FormattingUtilities.getSelectListPhrase((IntrospectedColumn)iter.next()));
            if(iter.hasNext())
                sb.append(", ");
            if(sb.length() > 80)
            {

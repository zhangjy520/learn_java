window.onload = function() {
		...
		$("#right").click(function(){
			leftCount++;
			//var leftsize = 0;  
	    	if($(".step3Ul").width()>1063){
	    		var leftsize= 0-1063*leftCount;
	    	
	        	$(".step3Ul").animate({"left": leftsize});  
	    	}else{
	    			//alert("数据不足");
	    		}
	    	});  
		...
		/*$(".h1").click(
			function() {
				...
				layer.open({
					...
					shadeClose: true,
					skin: 'yourclass',
					content: '<div class="fenBanFail"><div ><img align="absmiddle" src="${ctxStatic}/fb/image/star/fail.png"><a> 分班失败，上传数据人数不足以按照设置分类，请更改分班个数或者每班人数</a></div><input onclick="closeAlertDiv()" type="button" value="重新设置"/></div>'
				});

				
				 * layer.msg('<img align="absmiddle"
				 * src="image/app1.png">玩命提示中');
		...
		$(".h3").click(
			function() {
				var html = '<div class="upLoadFail"><div class="alertDivHeader"><label>上传失败</label></div><div class="alertDivContent"><label>可能原因如下：</label><br><br><div class="alertReason">1.上传文件格式不是excel格式。<br><br>2.上传的表格有合并的单元格。<br><br>3.上传的必填内容不完整。</div><div class="alertButtons"><input type="button" value=" 取消 "/><input type="button" value=" 重新上传 "/></div></div></div>';
		...
		$(".step3Table>table>tbody>tr").hover(
				function() {
					$(this).find("td[class='deleteTd']").html('<div onclick="deleteThis()" class="alertImg"><img align="AbsBottom" src="/k12platform/static/fb/image/delete1.png" /><a> 删除</a></div>');
...
// 关闭灰色 jQuery 遮罩
function closeBg() {
	$("#fullbg,.dataView").hide();
}

function showGradeBg(){
	var bh = $("body").height();
	var bw = $("body").width();
	$("#fullbg1").css({
		height: bh,
...
function closeGradeBg(){
	$("#fullbg1,.scoreSet").hide();
}
...
public abstract class VFSUtil {
	...
	private static void readVFSRootPath() {
		...
		try {
			...
			InputStream inStream = new ClassPathResource("/application.properties").getInputStream();
			p.load(inStream);
	...
	public static void moveFile(String src, String target, String newFileName, boolean isOverride) {
		...
		if (srcFile.isFile()) {
			...
			try {
				...
				throw new FileOperationException("移动文件【" + src + "】到目录【" + target + "】下失败，失败原因：" + ex.getMessage(), ex);
			}

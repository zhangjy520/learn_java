...
import cn.gukeer.divideStudent.sys.persistence.dao.OfficeDao;
...
//		getCacheMap().remove(key);
	 ...
	 * 导出Excel使用，根据名字转换为id
	 */
	/*public static Dept getByDeptName(String name){
		Dept a = new Dept();
		a.setMc(name);
		List<Dept> list = deptDao.findList(a);
		if(list.size()>0){
			return list.get(0);
		}else{
			return new Dept();

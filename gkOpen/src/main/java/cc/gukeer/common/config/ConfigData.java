...
public class ConfigData {
    ...
    // =================================
    private static Map<String, List<KVEntity>> map = new HashMap<String, List<KVEntity>>();

    // 手环状态 【0未激活, 1激活正常, 2:异常，3丢失，4脱腕】
    public static final String RING_STATUS_KEY = "ring_status_key";
    public static List<KVEntity> ringStatus = new ArrayList<KVEntity>();

    static {

        KVEntity entry = new KVEntity("0", "未激活");
        ringStatus.add(entry);
        entry = new KVEntity("1", "正常");
        ringStatus.add(entry);
        entry = new KVEntity("2", "异常");
        ringStatus.add(entry);
        entry = new KVEntity("3", "丢失");
        ringStatus.add(entry);
        entry = new KVEntity("4", "脱腕");
        ringStatus.add(entry);

    ...
    <table>
        ...
        <c:forEach items="${roleList}" var="role">
            <tr>
                <td>${role.name}</td>
                <td>
                    <input type="button" value="移除" onclick="delRoleFromApp(this,'${role.id}','${role.name}')">
                </td>
            </tr>
        </c:forEach>
    </table>
...
<div class="addDiv">
    <span>未分配角色</span>
    <ul class="app-ul">
        <c:forEach items="${allRoleList}" var="all" varStatus="num">
            <li class="app-li">${all.name}</li>
        </c:forEach>
    </ul>
</div>
...
<script>
    function delRoleFromApp(btn, id,name) {
        var _id = id;
        var _name = name;
        $.ajax({
            url: '${ctx}/app/delrolefromapp',
            type: 'POST',
            ...
            success: function (data) {
                if (!data) {
                    alertTips('300px', '150px', '提示', '移除角色失败,请刷新后重试!');
                    return;
                }

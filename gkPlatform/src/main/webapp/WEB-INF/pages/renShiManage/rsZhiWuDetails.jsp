	...
	<div class="mainContain fenbanMain fenbanStep">
			<div class="forBeauty" style="background:#ffffff;">
				<div class="rsTableHeader" style="height: 20px !important;">
					<div class="">
						<input class="rsButton" style="position: relative;float: right;margin-top: -5px;margin-right: 14px" onclick="openDialog('新增人员','${ctx}/renshi/title/teacher/add?titleId='+${title.id},'600px','400px');" type="button" name=""  value=" 新增 " />
						<%--<input class="rsButton delete" onclick="alertTips(400,200,'删除','确定要删除选中项吗？','sure()')" type="button" style="float: none;color: #fff;" value=" 删除 " />--%>
                        <span onclick="location.href='javascript:history.go(-1);'" id="backSpanB" style="font-size: 18px;color: #1AB394;" >
							返回
						</span>
						...
						<c:forEach items="${teacherList}" var="teacher" varStatus="status">
							<tr>
								...
								<td class="actionA">
									...
									<a class="deleteA" onclick="alertTips(400,222,'移除职位','确定要移除${teacher.name}的${currentTitle.mc}职务吗?','deleteSure(\'${teacher.id}\')')">移除</a>
								</td>
							</tr>
						</c:forEach>

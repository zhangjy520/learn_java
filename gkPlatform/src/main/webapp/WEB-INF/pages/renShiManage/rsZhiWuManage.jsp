...
<main class="container">
	<div class="row" id="stu-manage">
		<aside class="col-xs-3">
			<div><button onclick="openDialog('新增职务','${ctx}/renshi/title/add','500px','320px')">新增职务</button></div>
				...
				<div class="search-box">
					<p>
						<c:if test="${gukeer:notEmptyString(currentTitle)}">
							...
							<button onclick="openDialog('分配职务','${ctx}/renshi/teacher/manyDepart/add?titleId=${currentTitle.id}','950px','750px');">添加</button>
						</c:if>
					</p>
				</div>
				<div>
							...
							<c:forEach items="${teacherList}" var="teacher" varStatus="status">
								<tr>
									...
									<td>
										...
										<span class="remove" onclick="alertTips(400,202,'移除职位','确定要移除${teacher.name}的职务吗?','deleteSure(\'${teacher.id}\')')">移除</span>
									</td>
								</tr>
							</c:forEach>
						</tbody>
			...
			<section id="not-generate">
				<div id="column-manage-menu" style="display: block;">
					<div class="column-manage-operation">
						<button class="column-manage-edit" onclick="openDialog('编辑职务','${ctx}/renshi/title/add?titleId=${currentTitle.id}','600px','320px');">编辑</button>
						<button class="column-manage-delete" onclick="alertTips(400,202,'删除职务','确定要删除${title.mc}吗?','deleteTitle(\'${currentTitle.id}\')')">删除</button>
					</div>
					<table>
						<tbody>
						<tr>
	...
	<script>
		...
		$(function() {
			...
			$(".fenY").createPage({
				...
				backFn:function(p){
					...
					window.location.href = "${ctx}/renshi/bumen/index?pageNum="+p+"&pageSize=10"+"&teacherName="+encodeURI(encodeURI(name));
				}
			});
			</c:if>
			$(".gotoPage").click(function (){
				...
				if (pageNum <= 0 || pageNum >${pageInfo.pages}){
					...
					window.location.href = "${ctx}/renshi/bumen/index?pageNum="+$(".go").val()+"&pageSize=10";
				}
			});
		...
		function setFontCss(treeId, treeNode) {
			if (treeNode.id == "${currentTitle.id}")
				return {
					'padding-top':' 0','background-color': '#def7f5','color': 'black','height': '25px','opacity': '.8','width': '175px'
				};

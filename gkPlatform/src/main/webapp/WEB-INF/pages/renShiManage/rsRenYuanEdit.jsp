...
<div class="container">
    <form method="post" action="${ctx}/renshi/renyuan/save" id="inputForm">
        ...
        <ul>
            ...
            <li>
                <span class="fSpan">职工编号：</span>
                <input type="text" name="no" value="${teacher.no}">
            </li>
            <li>
                <span class="fSpan">手机号码：</span>
                <input type="text" name="mobile"  data-rule-mobile="true"  data-msg-mobile="手机格式错误" value="${teacher.mobile}">
            </li>
            </li>
            <li>
                <span class="fSpan">邮箱：</span>
                <input type="text" name="email"   data-rule-mail="true"  data-msg-mail="邮箱格式错误" value="${teacher.email}">
            </li>

            <li>
                <span class="fSpan">身份证号码：</span>
                <input type="text" name="identity"   data-rule-idCard="true"  data-msg-idCard="身份证格式错误" value="${teacher.identity}">
            </li>

            <li>
                <span class="fSpan">开始工作时间：</span>
                <input type="text" name="beginWork" id="beginWork" <c:if test="${gukeer:notEmptyString(teacher.startWork)}">value="${gukeer:millsToyyyyMMdd(teacher.startWork)}"</c:if> class="laydate-icon"/>
            </li>
            <li>
                ...
                <select name="zhiwu">
                    ...
                    <c:forEach items="${titleList}" var="title">
                        <option value="${title.id}" <c:if test="${teacher.titleId ==title.id}">selected</c:if>>${title.mc}</option>
                    </c:forEach>
                </select>
            </li>
           <%-- <li>
            ...
            <li>
                ...
                <input type="text" <c:if test="${gukeer:notEmptyString(teacher.highTime)}">value="${gukeer:millsToyyyyMMdd(teacher.highTime)}"</c:if> id="graduateDate" name="zgbysj" class="laydate-icon"/>
            </li>
            <li>
                <span class="fSpan">最高专业：</span>
                <input type="text" name="zgzy" value="${teacher.highJob}"/>
            </li>
            <li>
                <span class="fSpan">评职详细：</span>
                <input type="text" name="pzxx" value="${teacher.pzxx}"/>
            </li>
            <li>
                <span class="fSpan">家庭住址详细：</span>
                <input type="text" name="jtzzxx" value="${teacher.address}"/>
            </li>
            <li>
                <span class="fSpan">骨干教师级别：</span>
                <input type="text" name="ggjsjb" value="${gukeer:intToString(teacher.ggjsjb)}"/>
            </li>
            <li>
                <span class="fSpan">合同开始时间：</span>
                <input type="text" id="beginDate"  <c:if test="${gukeer:notEmptyString(teacher.htkssj)}">value="${gukeer:millsToyyyyMMdd(teacher.htkssj)}"</c:if> name="htkssj" class="laydate-icon"/>
            </li>
            <li>
                <span class="fSpan">曾用名：</span>
                <input type="text" name="cym" value="${teacher.cym}"/>
            </li>
            <li>
                <span class="fSpan">家庭邮编：</span>
                <input type="text" name="jtyb" value="${teacher.jtyb}"/>
            </li>
            <li>
                <span class="fSpan">是否专任教师：</span>
                <input type="text" name="sfzrjs" value="${gukeer:intToString(teacher.sfzrjs)}"/>
            </li>
            <li>
                <span class="fSpan">身份：</span>
                <input type="text" name="shenfen" value="${teacher.sf}"/>
            </li>
            <li>
                <span class="fSpan">外语水平：</span>
                <input type="text" name="wysp" value="${teacher.wysp}"/>
            </li>
            <li>
                <span class="fSpan">最高学制：</span>
                <input type="text" name="zgxz" value="${teacher.zgxz}"/>
            </li>
            <li>
                <span class="fSpan">学位数量：</span>
                <input type="text" name="xwsl" value="${teacher.xwsl}"/>
            </li>
            <li>
                <span class="fSpan">任教学科级别：</span>
                <input type="text" name="rjxkjb" value="${teacher.rjxkjb}"/>
            </li>
            <li>
                <span class="fSpan">校区：</span>
                <input type="text" name="xq" value="${teacher.xq}"/>
            </li>
            <li>
                <span class="fSpan">任教学科：</span>
                <input type="text" name="rjxk" value="${teacher.rjxk}"/>
            </li>
            <li>
                <span class="fSpan">实职级别：</span>
                <input type="text" name="szjb" value="${teacher.szjb}"/>
            </li>
            <li>
                <span class="fSpan">工资岗位(副)：</span>
                <input type="text" name="gzgwf" value="${teacher.gzgwf}"/>
            </li>
        ...
        <ul>
                ...
                <ul class="img-list-icon" style="width: 100px;height:100px;float: right;margin-right:141px;">
                    <li>
                        ...
                        <c:if test="${gukeer:emptyString(teacher.headUrl)}">
                            <img src="${ctx}/file/pic/show?picPath=${defaultHead}" data-url="${defaultHead}" width="100%" height="100%" id="head_url">
                        </c:if>
                        <c:if test="${gukeer:notEmptyString(teacher.headUrl)}">
                            <img src="${ctx}/file/pic/show?picPath=${teacher.headUrl}" data-url="${teacher.headUrl}" width="100%" height="100%" id="head_url">
                        </c:if>
                    </li>
                </ul>
            ...
            <li>
                ...
                <input type="text" name="xinzhi" value="${gukeer:intToString(teacher.salary)}"/>
            </li>
            <li>
                <span class="fSpan">岗位分类(副)：</span>
                <input type="text" name="gwflf" value="${teacher.gwflf}"/>
            </li>
            <li>
                <span class="fSpan">籍贯：</span>
                <input type="text" name="jg" value="${teacher.jg}"/>
            </li>
            <li>
            ...
            <li>
                ...
                <input type="text" id="firstGraduateDate" <c:if test="${gukeer:notEmptyString(teacher.ysbysj)}">value="${gukeer:millsToyyyyMMdd(teacher.ysbysj)}"</c:if>  name="ysbysj" class="laydate-icon"/>
            </li>
            <li>
                ...
                <select name="zgxl">
                    ...
                    <option value="中职/高中" <c:if test="${teacher.zgxl=='中职/高中'}">selected = "selected"</c:if>>中职/高中</option>
                    ...
                    <option value="硕士研究生" <c:if test="${teacher.zgxl=='硕士研究生'}">selected = "selected"</c:if>>硕士研究生</option>
                    <option value="博士研究生" <c:if test="${teacher.zgxl=='博士研究生'}">selected = "selected"</c:if>>博士研究生</option>
                </select>
                <%--<input type="text" name="zgxl" value="${teacher.zgxl}"/>--%>
            </li>
            <li>
                <span class="fSpan">最高毕业学校：</span>
                <input type="text" name="zgbyxx"  value="${teacher.zgbyxx}"/>
            </li>
            <li>
                <span class="fSpan">原专业：</span>
                <input type="text" name="yzy" value="${teacher.yzy}"/>
            </li>
            <li>
                <span class="fSpan">评职时间：</span>
                <input type="text" id="pzDate" name="pzsj" class="laydate-icon" <c:if test="${gukeer:notEmptyString(teacher.pzsj)}">value="${gukeer:millsToyyyyMMdd(teacher.pzsj)}"</c:if> />
            </li>
            <li>
                <span class="fSpan">来我校时间：</span>
                <input type="text" id="reportDate" name="lwxsj" class="laydate-icon" <c:if test="${gukeer:notEmptyString(teacher.lwxsj)}">value="${gukeer:millsToyyyyMMdd(teacher.lwxsj)}"</c:if>/>
            </li>
            <li>
                <span class="fSpan">住宅电话：</span>
                <input type="text" name="zzdh"  value="${teacher.zzdh}"/>
            </li>
            <li>
                <span class="fSpan">工资岗位：</span>
                <input type="text" name="gzgw"  value="${teacher.gzgw}"/>
            </li>
            <li>
                <span class="fSpan">合同结束时间：</span>
                <input type="text" id="finishDate" name="htjssj" class="laydate-icon"  <c:if test="${gukeer:notEmptyString(teacher.htjssj)}">value="${gukeer:millsToyyyyMMdd(teacher.htjssj)}"</c:if>/>
            </li>
            <li>
                <span class="fSpan">办公室电话：</span>
                <input type="text" name="bgsdh" value="${teacher.bgsdh}"/>
            </li>
            <li>
                <span class="fSpan">是否华侨：</span>
                <input type="text" name="sfhq" value="${teacher.sfhq}"/>
            </li>
            <li>
                <span class="fSpan">是否班主任：</span>
                <input type="text" name="sfbzr" value="${teacher.sfbzr}"/>
            </li>

            <li>
                <span class="fSpan">外语语种：</span>
                <input type="text" name="wyyz" value="${teacher.wyyz}"/>
            </li>
            <li>
                <span class="fSpan">原学制：</span>
                <input type="text" name="yxz" value="${teacher.yxz}"/>
            </li>
            <li>
                <span class="fSpan">最高学位：</span>
                <input type="text" name="zgxw" value="${teacher.zgxw}"/>
            </li>
            <li>
                <span class="fSpan">专业技术岗位分类：</span>
                <input type="text" name="zyjsgwfl" value="${teacher.zyjsgwfl}"/>
            </li>
